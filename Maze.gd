...
var up_door = preload("res://Tiles/UpDoor.tscn")
var right_door = preload("res://Tiles/RightDoor.tscn")
var left_door = preload("res://Tiles/LeftDoor.tscn")
var knight = preload('res://Enemies/Knight1.tscn')
var knight2 = preload('res://Enemies/Knight2.tscn')
var knight3 = preload('res://Enemies/Knight3.tscn')
var grid = []
var new_grid = []
var initial_grid = []
var fixed_rooms = []
...
func create_room(x1, x2, y1, y2, coords_x, coords_y):
	...
	if coords_y != 0:
		...
		for i in range(7):
			set_cell(mean_x-1, y1+6-i, 0, false, false, false, Vector2(randi()%3,1))
			set_cell(mean_x, y1+6-i, 0, false, false, false, Vector2(randi()%3,1))
			set_cell(mean_x+1, y1+6-i, 0, false, false, false, Vector2(randi()%3,1))
	...
	if coords_y != 7:
		...
		for i in range(mean_x-3, mean_x+3):
			...
			set_cell(i, y2+1, -1)
		$YSort/Foreground.set_cell(mean_x-1, y2, -1)
	if coords_x != 7:
		for i in range(8):
		...
		for i in range(mean_y-1, mean_y+4):
			set_cell(x2, i, 2, false, false, false, Vector2(0, (i-y1-1)%3))
		$YSort/Foreground.set_cell(x2, mean_y, -1)
	if coords_x != 0:
		for i in range(8):
		...
		$YSort/Foreground.set_cell(x1, mean_y, -1)
...
func _ready():
	for x in range(8):
		...
		for y in range(8):
			...
			grid[x][y].coords_x = x
			grid[x][y].coords_y = y
			grid[x][y].generate_individual_tiles()
			grid[x][y].spawn_painting()
			grid[x][y].generate_doors()
			grid[x][y].generate_enemies()
			add_child(room_instance)
			initial_grid[x].append([])
			initial_grid[x][y]=0
	var initial_room = grid[0][0]
	
	grid[0][1].x1 = initial_room.x1 + initial_room.width/2 - grid[0][1].width/2
	grid[0][1].y1 = initial_room.y1 + initial_room.height + 15
	grid[1][0].x1 = initial_room.x1 + initial_room.width + 9
	grid[1][0].y1 = initial_room.y1 + initial_room.height/2 - grid[1][0].height/2
	...
	create_room(grid[0][1].x1, grid[0][1].x1+grid[0][1].width, grid[0][1].y1, grid[0][1].y1+grid[0][1].height, grid[0][1].coords_x, grid[0][1].coords_y)
	create_room(grid[1][0].x1, grid[1][0].x1+grid[1][0].width, grid[1][0].y1, grid[1][0].y1+grid[1][0].height, grid[1][0].coords_x, grid[1][0].coords_y)
	grid[0][0].built = true
	grid[0][1].built = true
	grid[1][0].built = true
